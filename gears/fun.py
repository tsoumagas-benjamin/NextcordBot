...
class Fun(commands.Gear, name="Fun"):
    ...
    async def daily_meme(self):
        try:
            ...
            meme_embed: stoat.SendableEmbed = get_meme()
    ...
    async def guessme(self, ctx: commands.Context, *, name: str):
        ...
        # Guess user age
        age_data = await Client.get_text(f"https://api.agify.io/?name={name}")
        age = age_data["age"]
        embed_description = f"Predicted age: {age}\n"
        # Guess user gender
        gender_data = await Client.get_text(f"https://api.genderize.io/?name={name}")
        ...
        nation_data = await Client.get_text(f"https://api.nationalize.io/?name={name}")
    ...
    async def meme(self, ctx: commands.Context):
        ...
        meme_post = get_meme()
        await ctx.channel.send(embed=meme_post)
