...
class Information(commands.Cog, name="Information"):
    ...
    async def vote_add(
        self, reaction: nextcord.Reaction, user: nextcord.User | nextcord.Member
    ):
    ...
    async def ping(self, interaction: nextcord.Interaction):
        ...
        start_time = time()
        embed = nextcord.Embed(
            title="Response Times", color=nextcord.Colour.from_rgb(0, 128, 255)
        )
        end_time = time()
    ...
    async def poll(self, interaction: nextcord.Interaction, question: str):
        ...
        poll = nextcord.Embed(
            title=poll_title, color=nextcord.Colour.from_rgb(0, 128, 255)
        )
    ...
    async def rule(self, interaction: nextcord.Interaction, number: int):
        ...
        if db.rules.find_one({"_id": interaction.guild.id}) != None:
            ...
            embed = nextcord.Embed(
                title=f"{interaction.guild.name} Rule {number}",
    ...
    async def setrules(self, interaction: nextcord.Interaction, *, rules: str):
        ...
        db.rules.replace_one(
            ...
            {"_id": interaction.guild.id, "rules": rule_arr},
            upsert=True,
        )
    ...
    async def statistics(self, interaction: nextcord.Interaction):
        ...
        embed.add_field(
            name=f"Servers with {self.bot.user.name}: ",
            value=server_count,
            inline=False,
        )
        embed.add_field(
            name=f"{self.bot.user.name} serving: ",
