...
class Audit(commands.Cog, name="Audit Logs"):
    ...
    async def on_guild_channel_create(self, channel: nextcord.abc.GuildChannel):
        ...
        if isinstance(channel, nextcord.TextChannel):
            create_channel = nextcord.Embed(
                title="Text Channel Created", color=nextcord.Colour.green()
            )
            create_channel.add_field(name="Category", value=f"{channel.category}")
        elif isinstance(channel, nextcord.VoiceChannel):
            create_channel = nextcord.Embed(
                title="Voice Channel Created", color=nextcord.Colour.green()
            )
            create_channel.add_field(name="Category", value=f"{channel.category}")
        elif isinstance(channel, nextcord.CategoryChannel):
            create_channel = nextcord.Embed(
                title="Category Channel Created", color=nextcord.Colour.green()
            )
        elif isinstance(channel, nextcord.StageChannel):
            create_channel = nextcord.Embed(
                title="Stage Channel Created", color=nextcord.Colour.green()
            )
            create_channel.add_field(name="Category", value=f"{channel.category}")
        else:
            create_channel = nextcord.Embed(
                title="Forum Channel Created", color=nextcord.Colour.green()
            )
            create_channel.add_field(name="Category", value=f"{channel.category}")
        ...
        create_channel.set_footer(
            text=f"Channel ID: {channel.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_guild_channel_delete(self, channel: nextcord.abc.GuildChannel):
        ...
        if isinstance(channel, nextcord.TextChannel):
            delete_channel = nextcord.Embed(
                title="Text Channel deleted", color=nextcord.Colour.red()
            )
            delete_channel.add_field(name="Category", value=f"{channel.category}")
        elif isinstance(channel, nextcord.VoiceChannel):
            delete_channel = nextcord.Embed(
                title="Voice Channel deleted", color=nextcord.Colour.red()
            )
            delete_channel.add_field(name="Category", value=f"{channel.category}")
        elif isinstance(channel, nextcord.CategoryChannel):
            delete_channel = nextcord.Embed(
                title="Category Channel deleted", color=nextcord.Colour.red()
            )
        elif isinstance(channel, nextcord.StageChannel):
            delete_channel = nextcord.Embed(
                title="Stage Channel deleted", color=nextcord.Colour.red()
            )
            delete_channel.add_field(name="Category", value=f"{channel.category}")
        else:
            delete_channel = nextcord.Embed(
                title="Forum Channel deleted", color=nextcord.Colour.red()
            )
            delete_channel.add_field(name="Category", value=f"{channel.category}")
    ...
    async def on_guild_channel_update(
        self, before: nextcord.abc.GuildChannel, after: nextcord.abc.GuildChannel
    ):
        ...
        if isinstance(before, nextcord.TextChannel):
            update_channel = nextcord.Embed(
                title="Text Channel updated", color=nextcord.Colour.blurple()
            )
        elif isinstance(before, nextcord.VoiceChannel):
            update_channel = nextcord.Embed(
                title="Voice Channel updated", color=nextcord.Colour.blurple()
            )
        elif isinstance(before, nextcord.CategoryChannel):
            update_channel = nextcord.Embed(
                title="Category Channel updated", color=nextcord.Colour.blurple()
            )
        elif isinstance(before, nextcord.StageChannel):
            update_channel = nextcord.Embed(
                title="Stage Channel updated", color=nextcord.Colour.blurple()
            )
        else:
            update_channel = nextcord.Embed(
                title="Forum Channel updated", color=nextcord.Colour.blurple()
            )
        ...
        update_channel.set_footer(
            text=f"Channel ID: {before.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_guild_role_create(self, role: nextcord.Role):
        ...
        create_role = nextcord.Embed(
            title="Role Created", color=nextcord.Colour.green()
        )
        ...
        create_role.set_footer(
            text=f"Role ID: {role.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_guild_role_update(self, before: nextcord.Role, after: nextcord.Role):
        ...
        update_role = nextcord.Embed(
            title="Role Updated", color=nextcord.Colour.blurple()
        )
        ...
        if before.hoist != after.hoist:
            update_role.add_field(
                name="Hoisted", value=f"{before.hoist} -> {after.hoist}"
            )
        if before.managed != after.managed:
            update_role.add_field(
                name="Integration Role", value=f"{before.managed} -> {after.managed}"
            )
        ...
        # Format the time the role was created at and the role ID into the footer
        update_role.set_footer(
            text=f"Role ID: {before.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_guild_update(self, before: nextcord.Guild, after: nextcord.Guild):
        ...
        update_server = nextcord.Embed(
            title="Server Updated", color=nextcord.Colour.blurple()
        )
        ...
        # Format the time the role was created at and the role ID into the footer
        update_server.set_footer(
            text=f"Server ID: {before.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_guild_emojis_update(
        ...
        before: Sequence[nextcord.Emoji],
        after: Sequence[nextcord.Emoji],
    ):
        # If there is no assigned audit log role for this server, return before creating an embed
        ...
        if len(before) > len(after):
            emoji_update = nextcord.Embed(
                title="Emoji Deleted", color=nextcord.Colour.red()
            )
            ...
            emoji_update.set_footer(
                text=f"Emoji ID: {deleted_emoji[0].id} | {self.date_format(datetime.now())}"
            )

        elif len(before) < len(after):
            emoji_update = nextcord.Embed(
                title="Emoji Created", color=nextcord.Colour.green()
            )
            # Get emojis that have been created and add them to the embed
            created_emoji = [emoji for emoji in after if emoji not in before]
            emoji_update.add_field(
                name="New Name",
                value=f"<:{created_emoji[0].name}:{created_emoji[0].id}> {created_emoji[0].name}",
            )
            emoji_update.set_footer(
                text=f"Emoji ID: {created_emoji[0].id} | {self.date_format(datetime.now())}"
            )
            ...
            for emoji_id, old_emoji in before_emojis.items():
                if emoji_id in after_emojis:
                    ...
                    if old_emoji.name != new_emoji.name:
                        emoji_update = nextcord.Embed(
                            title="Emoji Updated", color=nextcord.Colour.blurple()
                        )
                        emoji_update.add_field(name="Old Name", value=old_emoji.name)
                        emoji_update.add_field(
                            name="New Name",
                            value=f"<:{new_emoji.name}:{new_emoji.id}> {new_emoji.name}",
                        )
                        emoji_update.set_footer(
                            text=f"Emoji ID: {emoji_id} | {self.date_format(datetime.now())}"
                        )
    ...
    async def on_member_update(self, before: nextcord.Member, after: nextcord.Member):
        ...
        # Check if a role has been removed
            ...
            member_update.add_field(
                name=removed_role[0].name, value=removed_role[0].mention
            )
        ...
        # Check if the user's username has changed
            ...
            user_update.add_field(
                name=f"{before.display_name} -> {after.display_name}",
                value=after.mention,
            )
    ...
    async def on_member_ban(self, guild: nextcord.Guild, user: nextcord.User):
        ...
        member_ban.set_footer(
            text=f"Member ID: {user.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_message_delete(self, message: nextcord.Message):
        ...
        if message.content:
            message_delete.add_field(
                ...
                value=f"Deleted message by {message.author.mention}",
            )

        # Set the thumbnail and footer
        message_delete.set_thumbnail(message.author.display_avatar.url)
        message_delete.set_footer(
            text=f"Message ID: {message.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_message_edit(self, before: nextcord.Message, after: nextcord.Message):
        ...
        if before.content is not after.content:
            message_edit.add_field(
                name=f"{before.content} -> {after.content}",
                value=f"Edited by {after.author.mention}",
            )

        # Set the thumbnail and footer
        message_edit.set_thumbnail(after.author.display_avatar.url)
        message_edit.set_footer(
            text=f"Message ID: {after.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_member_join(self, member: nextcord.Member):
        ...
        member_join = nextcord.Embed(
            title="Member Joined", color=nextcord.Colour.green()
        )
        member_join.add_field(
            name=f"{member.display_name} #{member.guild.member_count}",
            value=f"Created at: {member.created_at}",
        )

        # Set the thumbnail and footer
        member_join.set_thumbnail(member.display_avatar.url)
        member_join.set_footer(
            text=f"Member ID: {member.id} | {self.date_format(datetime.now())}"
        )
    ...
    async def on_member_remove(self, member: nextcord.Member):
        ...
        member_remove.add_field(
            name=f"{member.display_name}",
            value=f"Joined at: {self.date_format(member.joined_at)}",
        )
