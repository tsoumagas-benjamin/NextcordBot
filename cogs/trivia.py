    ...
    async def callback(self, interaction: nextcord.Interaction):
        if self.values[0] == self.right:
            ...
            await interaction.response.send_message(
                f"{self.values[0]} is correct!", ephemeral=True
            )
        else:
            await interaction.response.send_message(
                f"{self.values[0]} is incorrect! Correct answer is {self.right}",
                ephemeral=True,
            )
...
class TriviaSetup:
    def __init__(self):
        ...
        self.embed: nextcord.Embed = nextcord.Embed(
            title="Trivia Results", color=nextcord.Colour.from_rgb(0, 128, 255)
        )
    ...
    async def trivia(self, interaction: nextcord.Interaction):
        ...
        for x in range(0, 10):
            ...
            await interaction.edit_original_message(
                content=content,
                view=TriviaView(t.incorrects[x], t.corrects[x], t.score, 10),
            )
            await sleep(10)
